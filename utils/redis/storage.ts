...
async function withRedisRetry<T>(operation: (client: Awaited<ReturnType<typeof getRedisClient>>) => Promise<T>): Promise<T> {
    const maxRetries = 3;
    let lastError: any = null;

    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            // Get Redis client (this handles reconnection if needed)
            const client = await getRedisClient();

            // Make sure client is ready
            if (!client.isReady) {
                console.log(`Redis client not ready on attempt ${attempt}/${maxRetries}, waiting...`);
            ...
            // If connection is lost, wait longer to allow reconnection
            if (error.message.includes('Socket closed') ||
                error.message.includes('Connection lost') ||
                error.message.includes('NOAUTH')) {
                console.log(`Connection issue detected, waiting longer before retry ${attempt}`);
                await new Promise(resolve => setTimeout(resolve, 1000 * attempt));
            } else if (attempt < maxRetries) {
                // For other errors, use standard backoff
                const delay = Math.min(Math.pow(2, attempt) * 100, 1000);
    ...
    console.error('All Redis operation retry attempts failed');
    throw lastError;
}

// Thread Functions
export async function getThreadsForUser(uid: string) {
    return withRedisRetry(async (client) => {
